...
export const crearPersona = async (socket: SocketIO.Socket, io: Socket) => {
  socket.on("persona:nueva", async (persona: Personas) => {
    const personaguardar = new PersonasDB(persona);

    const salt = await bcrypt.genSalt(10);

    if (salt) {
      const hash = await bcrypt.hash(personaguardar.codigo, salt);

      if (hash) {
        personaguardar.password = hash;
...
const encryptPasswod = (persona: object) => {};
